...
function Login({ changeForm }) {
  ...
  const validateForm = (data) => {
    ...
    if (data.password || data.password.length == 0){
      ...
      if (!validPwd){
        toast.error('password should be at least 6 characters long and must contain capital letter and symbol', {
          position: 'bottom-right',
          draggable: true,
          autoClose: 6000,
          pauseOnHover: true,
  ...
  return (
    <Container>
      ...
      <Input placeholder='username or email' onChange={(e) => setUsrEmail(e.target.value)}/>
      <Input type='password' placeholder='password' onChange={(e)=> setPassword(e.target.value)}/>
      <SubmitButton onClick={submitForm}>Sign In</SubmitButton>
      <Span>
        Not registerd yet? 
        <Span style={{color: 'green', cursor: 'pointer', paddingLeft: '.3rem'}} onClick={()=>changeForm()}>
           Register
        </Span>
      </Span>
      <ToastContainer />
